...
`env` block of a configured stdio MCP server. **This extension stores nothing** — there is no
field to type a key into, deliberately, so a secret cannot end up in plugin state. With no key
